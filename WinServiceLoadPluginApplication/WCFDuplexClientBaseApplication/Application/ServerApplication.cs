...
namespace WCFDuplexClientBaseApplication
{
    ...
    public class ServerApplication
    {
        ...
        /// 插件列表
        /// </summary>
        public static List<PluginBase> Plugins { get; set; }
        ...
        public static void StartApplication()
        {
            ...
            //加载运行插件及其任务线程
            ServerApplication.LoadPlugin();//实现加载插件并运行任务线程
        ...
        public static void LoadPlugin()
        {
            ...
            string startLogPath = AppDomain.CurrentDomain.BaseDirectory + "start.log";
            if (File.Exists(startLogPath))
            {
                File.Delete(startLogPath);
            }
            StreamWriter logsw = new StreamWriter(startLogPath, true);
            try
            {
                //加载服务插件
                ServerApplication.Plugins = new List<PluginBase>();
                ...
                foreach (DirectoryInfo item in dirs)
                {
                    ...
                    st.Start();
                    PluginBase plugin = ServerApplication.ReadFramePluginItem(item.FullName);
                    st.Stop();
                    if (plugin != null)
                    {
                        ServerApplication.Plugins.Add(plugin);
        ...
        /// <returns></returns>
        public static PluginBase ReadFramePluginItem(string pluginPath)
        {
            AppDomainSetup appDomainSetup = new AppDomainSetup();
            appDomainSetup.ApplicationName = pluginPath;
            appDomainSetup.ApplicationBase = pluginPath;
            //启用影像赋值程序集
            appDomainSetup.ShadowCopyFiles = "true";
            appDomainSetup.ShadowCopyDirectories = appDomainSetup.ApplicationBase;
            AppDomain appDomain = AppDomain.CreateDomain(pluginPath,null,appDomainSetup);
            ...
            {
                PluginBase instance = factory.LoadFramePlugin(item, "WCFDuplexClientBaseApplication.PluginBase") as PluginBase;
                if (instance!=null)
                {
                    instance.Initialize(item);
                    instance.Initialize();
                    instance.PluginDomain = appDomain;
                    instance.FrameDomain = AppDomain.CurrentDomain;
                    return instance;
                }
            }
        ...
        {
            //销毁插件及任务线程
            foreach (var plugin in ServerApplication.Plugins)
            {
                try
                {
                    ...
                    AppDomain.Unload(plugin.FrameDomain);
                }
                catch (Exception)
                {
                ...
                //停止端口服务
                foreach (var host in ServerApplication.ServiceHosts)
                {
                    host.Close();
                }
            }
