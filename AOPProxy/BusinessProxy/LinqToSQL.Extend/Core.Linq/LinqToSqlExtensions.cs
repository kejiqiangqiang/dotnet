...
namespace BusinessProxy.LinqToSQL.Extend.Core.Linq
{
	...
	{ 
		...
		private static SqlCommand CombineCommands( List<DbCommand> selectCommands )
		{
            ...
            System.Data.SqlClient.SqlParameterCollection newParamList = batchCommand.Parameters;
		...
		public static string GetDbSetStatement<TEntity>( MemberInitExpression memberInitExpression, Table<TEntity> table, DbCommand updateCommand ) where TEntity : class
		{
            var entityType = typeof(TEntity);
            //快速编译对象
            FastLambda.FastEvaluator s =FastEvaluatorCache.FastEvaluator;
            object entity = s.Eval(memberInitExpression);
            //Expression.Lambda( memberInitExpression, null).Compile().DynamicInvoke(null);//
            ...
            foreach (var binding in memberInitExpression.Bindings)
            {
                ...
                var dbCol = (from mdm in metaTable.RowType.DataMembers
                             where mdm.Name == name
                             select new { mdm.MappedName, mdm.Name }).FirstOrDefault();
                if (dbCol == null)
                {
                ...
                {
                    //反射调用求值
                    //object constant = entityType.GetProperty(dbCol.MappedName).GetValue(entity, null);
                    object constant = s.Eval(assignment.Expression);
                ...
                {
                    //公式表达式: TEntity.Property=TEntity.Property + " Hello"
                    var selectExpression = Expression.Call(typeof(Queryable), "Select", new Type[] { entityType, assignment.Expression.Type },
                     Expression.Constant(table), Expression.Lambda(assignment.Expression, entityParam));
                    setSB.AppendFormat("[{0}] = {1}, ", dbCol.MappedName, GetDbSetAssignment(table, selectExpression, updateCommand, name));
                }
            }
            var setStatements = setSB.ToString();
            return "SET " + setStatements.Trim().TrimEnd(",".ToArray()); // remove ', '
		}
