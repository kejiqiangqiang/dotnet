...
namespace WindowsFormsWCFHostServer.Hosting
{
    ...
    {
        public static ServiceHost StartServiceHosting()
        {
            ServiceHost host = new ServiceHost(typeof(Service1));

            NetTcpBinding binding = new NetTcpBinding();

            Uri baseAddress = new Uri("net.tcp://localhost:8733/Design_Time_Addresses/WCFServiceDuplex/Service1/");
            host.AddServiceEndpoint(typeof(IService1), binding, baseAddress);

            
                ServiceMetadataBehavior behavior = new ServiceMetadataBehavior();
                behavior.HttpGetEnabled = true;
                behavior.HttpGetUrl = new Uri("http://localhost:8734/Design_Time_Addresses/WCFServiceDuplex/Service1/mex");
        ...
        public static ServiceHost HttpHost(Type instanceType, Type interfaceType, string wcfUri)
        {
            ...
            try
            {
                ...
                //跨越访问
                ServiceHosting.Accesspolicy(host, wcfUri);
            }
            catch (Exception ex)
            {
